...
from socio4health.utils.harmonizer_utils import translate_column
...
def test_basic_translation(mock_translate) -> None:
    ...
    result = translate_column(df, column="text", language="fr")
...
def test_handles_null_values(mock_translate) -> None:
    ...
    result = translate_column(df, column="text")
...
def test_invalid_input_type() -> None:
    with pytest.raises(TypeError, match="data must be a pandas DataFrame"):
        translate_column(["not", "a", "DataFrame"], column="text")
